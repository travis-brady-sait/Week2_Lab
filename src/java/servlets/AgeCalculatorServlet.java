...
public class AgeCalculatorServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try
        {
            myAge2 = Integer.parseInt(myAge);
        }
        catch(NumberFormatException e)
        {
            request.setAttribute("result", "<p>You must enter a number.</p>");
            getServletContext().getRequestDispatcher("/agecalculator.jsp").forward(request, response);
        }
        request.setAttribute("myAge", myAge);
        request.setAttribute("currentAge", myAge2);
        myAge2 = myAge2 + 1;
        request.setAttribute("result", "<p>Your age next birthday will be " + myAge2 + "!</p>");
        getServletContext().getRequestDispatcher("/agecalculator.jsp").forward(request, response);
        }
        else if (myAge == "")
        {
            request.setAttribute("result", "<p>You must give your current age.</p>");
            getServletContext().getRequestDispatcher("/agecalculator.jsp").forward(request, response);
        }
        else 
        {
